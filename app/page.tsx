...
export default function Home() {
  ...
  return (
    <main>
      ...
      {settingsOpen && <div className="modal-wrap" onClick={() => setSettingsOpen(false)}><section className="settings-modal" role="dialog" aria-modal="true" aria-labelledby="settings-title" onClick={(event) => event.stopPropagation()}><div className="drawer-head"><div><span className="section-kicker">تخصيص بسيط</span><h2 id="settings-title">الإعدادات</h2></div><button className="icon-close" onClick={() => setSettingsOpen(false)} aria-label="إغلاق الإعدادات"><Icon name="close" /></button></div><div className="settings-profile"><div className="profile-avatar">{user?.email.slice(0, 1).toUpperCase()}</div><div><strong>{user?.email}</strong><small>حساب تجريبي محلي</small></div></div><label className="setting-row"><span><strong>رسائل المنتجات الجديدة</strong><small>أرسلوا لي مفاجآت وكتالوجات جديدة</small></span><input type="checkbox" checked={emailUpdates} onChange={(event) => setEmailUpdates(event.target.checked)} /></label><div className="orders-heading"><strong>آخر الطلبات</strong><span>{orders.length} طلب</span></div>{orders.length === 0 ? <p className="orders-empty">لم تنفذ طلبًا بعد. ستظهر عملياتك هنا بعد الدفع التجريبي.</p> : <div className="orders-list">{orders.slice(0, 5).map((order) => <button className="order-row" key={order.id} onClick={() => setSelectedOrder(order)}><div><strong>{order.id}</strong><small>{new Intl.DateTimeFormat("ar-SA", { dateStyle: "medium" }).format(new Date(order.createdAt))} · {order.itemCount} منتجات</small><small className="order-items">{order.items?.map((item) => `${item.name} ×${item.quantity}`).join("، ") || "تفاصيل قديمة محفوظة قبل تحديث السجل"}</small></div><div><b>{priceText(order.total)} ر.س</b><small>{order.status}</small></div></button>)}</div>}<div className="settings-note">يمكن ربط الإشعارات وتسجيل الدخول الحقيقي عند إضافة مزود مصادقة مثل Supabase أو Clerk.</div><button className="button-dark full-button" onClick={() => setSettingsOpen(false)}>حفظ والعودة</button></section></div>}
