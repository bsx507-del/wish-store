...
export default function Home() {
  ...
  useEffect(() => {
    ...
    void supabase.auth.getSession().then(({ data }) => {
      if (mounted && data.session?.user.email) {
        setUser({ email: data.session.user.email, joinedAt: data.session.user.created_at });
      }
    });
    const { data: listener } = supabase.auth.onAuthStateChange((_event, session) => {
      if (session?.user.email) setUser({ email: session.user.email, joinedAt: session.user.created_at });
  ...
  const shareProduct = async (product: Product) => {
    ...
    const shareData = { title: `${product.name} · وِشّ`, text: `${product.name} بسعر ${priceText(product.price)} ر.س — منتج افتراضي للمتعة فقط`, url };
    try {
      if (navigator.share) {
  ...
  const signInWithGoogle = async () => {
    if (!supabase) {
      setAuthError("أضف إعدادات Supabase في Vercel أولًا لتفعيل Google.");
      return;
    }
    ...
    if (error) setAuthError("تعذر فتح تسجيل Google. تأكد من إعداد رابط الرجوع في Supabase.");
  };
  ...
  const payOrder = () => {
    if (paymentMethod !== "wallet" || isPaying) return;
    setIsPaying(true);
